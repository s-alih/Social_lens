...
/**
 * @title LensHubStorate
 * @author Lens Protocol
 *
 * @notice This is an abstract contract tha *only* contains for the LensHub contract. This
 * *must* be inherited last (bar interfaces) in order to preserve LensHub storage layout. Adding
 * storate variable should be done solely at the bottom of this contract
 */
...
abstract contract LensHubStorage {
    ...
    mapping(uint256 => address) internal _dipatcherByProfile;
    mapping(bytes32 => uint256) internal _profileIdByHanleHash;
    mapping(uint256 => DataTypes.ProfileStruct) internal _profileById;
    mapping(uint256 => mapping(uint256 => DataTypes.ProfileStruct))
        internal _pubByIdProfile;

    mapping(address => uint256) internal _defaultProfileByAddress;

    uint256 internal _profileCouter;
