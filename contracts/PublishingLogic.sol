 ...
 *
 * @dev The function are external, so they are called from the hub via `delegateCall` under the hood. Furthermore.
 * expected events are emitter from this library instaed of from the hubto alleviate code size concerns.
 *
 */
...
library PublishingLogic {
     ...
     *
     * @param vars the CreateProfileData struct containing the followner parameters:
     ...
     * @param _profileIdByHandleHash The storage reference to the mapping of the profile IDs by handle hash.
     * @param _profileById The storage reference to the mapping of profile structs by IDs.
     * @param _followModuleWhitelisted The storage reference to the mapping of whitelisted status by follow module address.
     */
    function createProfile(
        ...
        mapping(addresss => bool) storage _followModuleWhitelisted
    ) external {
        ...
        bytes handleHash = keccak256(bytes(vars.handle));
        ...
        bytes memory followModuleReturnedData;
        ...
        _emiteProfileCreated(profileId, vars, followModuleReturnedData);
    }
     ...
     * @param _profile The storage reference to the profile struct associated with the given profiel ID.
    ...
    /**
     * @notice Creates a post publication mapped to given profile
     * 
     * @dev To avoid a stack too deep error, reference parameters are passed in memory rather than calldata.
     * 
     * @param profileId The profile ID to associated this publication to.
     ...
     * @param referenceModuleInitData The data to pass to the reference module for publication initializalation.
     * @param pubId The publication ID to associate with this publication.
     * @param _pubByIdByProfile The storage reference to the mapping of publication by publication ID by Profile ID.
     * @param _collectModuleWhitelisted The storage reference to the mapping of whitelisted status by collect module address.
     * @param _referenceModuleWhitelistd The storage reference to the mapping of whitelisted status  by reference module address.
     */
    ...
    function createPost(
        ...
        uint256 pubId,
        mapping(uint256 => mapping(uint256 => DataTypes.PublishingStruct)) storage _pubByIdProfile,
        mapping(address => bool) storage _collectModuleWhitelisted,
        mapping(address => bool) storage _referenceModuleWhitelistd,
    ) external {
    ...
    /**
     * @notice Creates a comment publication mapped to the the given profile.
     * 
     * @dev This function is unique in that it requires many variables, so, unlike other publishing functions,
     * we need to pass th full commentData struct in memory to avoid a stack too deep error.
     * 
     * @param vars The commentData struct to use to create the comment.
     * @param pubId The publication ID to associated with this function.
     * @param _profileById The storage reference to the mapping of profile structs by IDs.
     * @param _pubByIdByProfile The storage reference to the mapping of publications by publicatin ID by profile ID.
     * @param _collectModuleWhitelisted The storge reference to the mapping of whitelisted status by collect module address.
     * @param _referenceModuleWhitelisted The storage reference to the mapping of whitelistd status by reference module address.
     */
    ...
    function createComment(
        ...
        mapping(address => bool) storage _collectModuleWhitelisted,
        maaing(address => bool) storage _referenceModuleWhitelisted
    ) external {
        // validated existence of the pointed publication
        uint256 pubCount = ProfileById[vars.profileIdPointed].pubCount;
        ...
        // Collect Module Initialization
        bytes memory collectModuleReturnData = _initialPubCollectModule(
            vars.profileId,
            pubId,
            vars.collectModule,
        ...
        if(refModule != address(0)){
            IReferenceModuel(refModule).processComment(
                vars.profileId,
     ...
     * @param pubId The publication ID to associated with this publication.
     * @param _pubByIdByprofile The storage reference to the mapping of publications by publication ID
     * @param _referenceModuleWhitlisted The storage reference to the mapping of whitelisted status by reference module address.
     * 
     */
    ...
    function createMirror(
        ...
        // Reference module validation
        address refModule = _pubByIdProfile[rootProfileIdPointed][
            rootPubIdPointed
        ].referenceModule;

        if(refModule != address(0)){
            IReferenceModuel(refModule).processMirror(
                vars.profileId,
    ...
    function _initPubCollectModule(
        ...
        return ICollectModule(collectModule).initializePublicationCollectModule(
            profileId,
            pubId,
    ...
    function _initPubReferenceModule(
        ...
        if(!_referenceModuleWhitlewhitelisted[referenceModule])
            revert Errors.ReferenceModuleNotWhitelisted();
        _pubByIdByProfile[profileId][pubId].referenceModule = referenceModule;
        return IReferenceModuel(referenceModule).inititalizeReferenceModule(
            profileId,
            pubId,
            referenceModuleInitData
        );
    ...
    function _validateHandle(string calldata handle) private pure{
        bytes memory byteHandle = bytes(handle);
         if (
            byteHandle.length == 0 ||
            byteHandle.length > Constants.MAX_HAN
