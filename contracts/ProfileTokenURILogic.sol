...
library ProfileTokenURILogic {
     ...
     *
     * @dev The decoded token URI JSON metadat contains the following fields: name, description, image and attribute.
     * The image field contain a base64-encoded SVG. Both the JSON metadata and the image generated fully on-chain.
     *
     *
     * @param id The token ID of the profile.
     * @param follower The number of the profile's followers.
     * @param owner The address which own the profile
    ...
    /**
     * @notice Generate the token image
     *
     * @dev If the image URI was set and meets URI formate conditions. it will be embedded in the token image.
     * Otherwise, a default picture will be used. Handle font size is function of handle length
     *
     * @param handleWithAtSymbol The profile's handle begnining with "@" symbol.
     * @param imageURI The profile's picture URI, An empty string if not been set.
     *
    ...
    /**
     * @notice Gets the fragment of th SVG correspondign to the profile picture.
     *
     * @dev If the image URI was set and meets URI format conditions. Theis will return an image tag referencing  it.
     ...
     *          fontSize(handleLength) = 24 - (hanleLength -12)/2 when handlelength > 17
     *
     * @param handleLength The profile's handle length
     * @param uint256 The font size.
     */
    ...
    /**
     * @notice Decides if profile NFT should use user provided custom profile picture or the defalult one.
     * 
