 ...
 * 
 * @notice This is the library that contains the logic for follow & collects.
 * 
...
library InteractionLogic{
    ...
    /**
     * @notice Follows the given profiles, exicuting the necessary logic and module calls before minting the follow
    ...
    *
    ...
    function follow(
        ...
        for(uint256 i= 0; i<proflieIds.length;){
    ...
    /**
     * @notice Collects the given publication, executing the necessary logic and module call before minting the collect NFT 
     * to the collector
     * 
     ...
     * @param collectNFTImpl The address of the collect NFT implementaion
     * @param _pubByIdByProfile A pointer to the storage mapping of publications by pubId by profile ID.
     * @param _profileById A pointer to the storage mapping of the profile structs by profile ID.
     * 
     * @return uint256 An integer representing the mintd token ID.
     */
     ...
     * @param pubId The publication ID of the publication being collected, which collects NFt should be deployed.
    ...
    function _deployCollectNFT(
        ...
        bytes firstBytes  = bytes4(bytes(handle));
    ...
    /**
     * @notice Emits the `Collected` event that sginals that a successful collect action has occured.
     * 
     * @dev The is done through this function to prevent stack too deep compilation error.
     * 
     * 
     * @param collector The address collecting the publication.
     * @param profileId The token ID of the profile that the collect was initiated towards, useful
